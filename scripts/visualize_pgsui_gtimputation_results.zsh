...
DEFAULT_VALIDATION_ROOT="${SCRIPT_DIR:h}"
IS_HPC_BUNDLE=false
if [[ -f "${DEFAULT_VALIDATION_ROOT}/manifests/pgsui_gpu_tasks.tsv" ]]; then
  IS_HPC_BUNDLE=true
elif [[ ! -d "${DEFAULT_VALIDATION_ROOT}/inputs/test-vcf-files" ]] \
  && [[ -d "${DEFAULT_VALIDATION_ROOT:h}/inputs/test-vcf-files" ]]; then
  DEFAULT_VALIDATION_ROOT="${DEFAULT_VALIDATION_ROOT:h}"
fi
...
PGSUI_BACKEND="${PGSUI_BACKEND:-cuda}"
RUNTIME_PGSUI_BACKENDS=("${(@s: :)${RUNTIME_PGSUI_BACKENDS:-${DEFAULT_RUNTIME_BACKENDS}}}")
...
exec "${PYTHON_BIN}" "${SCRIPT_DIR}/visualize_pgsui_gtimputation_results.py" \
  ...
  --runtime-pgsui-backends "${RUNTIME_PGSUI_BACKENDS[@]}" \
  "$@"
