...
def unzip_nested_zip_seqfiles(
    zip_ref, storage_path, extension_list, i, outdir, counter
):
    ...
    for filename in zip_ref.namelist():
        ...
        if filename.endswith(".zip"):
            ...
            with zipfile.ZipFile(zfiledata) as zip_ref2:
                counter = unzip_nested_zip_seqfiles(
                    zip_ref2, storage_path, extension_list, i, outdir, counter
                )
    return counter
...
zipfiles = [f"training_data/download/{z}" for z in zipfiles]

counter = 0
for cnt, myzip in enumerate(tqdm(zipfiles, desc="Extracting Zipfiles: "), start=1):
    outdir = f"{storage_path}/results{cnt}"
    Path(outdir).mkdir(parents=True, exist_ok=True)

    try:
        with zipfile.ZipFile(myzip, "r") as zip_ref:
            counter = unzip_nested_zip_seqfiles(
                zip_ref, storage_path, extension_list, cnt, outdir, counter
            )

    except zipfile.BadZipFile:
        continue

print(f"Extracted {counter} total files")
