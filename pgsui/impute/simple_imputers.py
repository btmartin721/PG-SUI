...
class ImputePhyloTransformer(GenotypeData, BaseEstimator, TransformerMixin):
    ...
    Attributes:
        tree_ (toytree.tree): Toytree object.
        data_ (Dict[str, Any]): Input data to be imputed.
        q_ (pandas.DataFrame): Rate Matrix Q.
        site_rates_ (pandas.DataFrame): Site rates.
    ...
    def fit(self, X):
        ...
        Args:
            X (GenotypeData): Instantiated GenotypeData object with data to impute.
        """
    ...
    def transform(self, X):
        ...
        Returns:
            GenotypeData: New GenotypeData instance with imputed data.
        """
        imputed012 = self.impute_phylo(
            self.tree_, self.data_, self.q_, self.site_rates_
        )

        imputed_filename = X.decode_imputed(
            imputed012, write_output=True, prefix=self.prefix
        )
        ...
        return GenotypeData(
            filename=imputed_filename,
            filetype=ft,
            popmapfile=X.popmapfile,
            guidetree=X.guidetree,
            qmatrix_iqtree=X.qmatrix_iqtree,
            qmatrix=X.qmatrix,
            siterates=X.siterates,
            siterates_iqtree=X.siterates_iqtree,
            verbose=False,
        )
    ...
    def _parse_arguments(
        ...
        if genotype_data is not None:
            ...
            self.parse_filetype(filetype_, genotype_data.popmapfile)
