...
class SafeFocalCELoss(nn.Module):
    ...
    Implements focal loss. The loss for each example is computed as follows.

    .. math:: FL = \alpha_y \cdot (1 - p_t)^\gamma \cdot CE

    where CE = -log(p_t), p_t = softmax(logits)[range(N), y].

    Notes:
        - If weight is provided, it is used as alpha_y (per-class scaling).
        - Reduction uses a weighted mean (divide by sum(alpha_y)) so that gamma=0 behaves like PyTorch's weighted CE mean.
    """
    ...
    def __init__(
        ...
        ignore_index: int = -1,
        eps: float = 1e-8,
        normalize_by: str = "alpha",  # "alpha" or "count" or "alpha_focal"
    ) -> None:
        """Initialize SafeFocalCELoss.

        This loss function computes the focal cross-entropy loss with support for ignoring specific target indices and includes numeric stability measures.

        Args:
            gamma (float): Focusing parameter. Default is 2.0.
            weight (torch.Tensor | None): Per-class weights (shape: [C]) used as alpha_y. Default is None.
            ignore_index (int): Target value ignored in loss computation. Default is -1.
            eps (float): Small constant for numeric stability. Default is 1e-8.
            normalize_by (str): Normalization strategy; "alpha" divides by sum(alpha_y; recommended, matches weighted CE at gamma=0);  "count" divides by number of valid samples; "alpha_focal" divides by sum(alpha_y * focal). Default is "alpha".
        """
        super().__init__()
        ...
        if weight is None:
            ...
            # Ensure float tensor; buffer moves with .to(device)
            self.register_buffer("weight", weight.detach().long())
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            ...
            targets (torch.Tensor): Shape (N,), integer class labels.

        Returns:
            torch.Tensor: Scalar loss as a float tensor.
        """
        ...
        logp = F.log_softmax(logits_v, dim=-1)  # (Nv, C)
        logp_t = logp.gather(1, targets_v.unsqueeze(1)).squeeze(1)  # (Nv,)
        p_t = logp_t.exp()

        # Standard CE per example: -log(p_t)
        ce = -logp_t
        ...
        if self.weight is None:
            loss_vec = focal * ce
            denom = (
                loss_vec.new_tensor(loss_vec.numel())
                if self.normalize_by == "count"
                else loss_vec.new_tensor(loss_vec.numel())
            )
            loss = loss_vec.sum() / denom.clamp_min(self.eps)
        else:
            weight = cast(torch.Tensor, self.weight)
            alpha = weight.index_select(0, targets_v)  # (Nv,)
            loss_vec = alpha * focal * ce

            if self.normalize_by == "alpha":
                denom = alpha.sum()
            elif self.normalize_by == "alpha_focal":
                denom = (alpha * focal).sum()
            else:  # "count"
                denom = loss_vec.new_tensor(loss_vec.numel())
