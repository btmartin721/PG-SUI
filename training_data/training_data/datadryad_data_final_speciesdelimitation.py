...
def download(origin, doi, num):
	...
	r2 = requests.get(Pcurl2, headers=Bheaders2, stream=True)

	with open("training_data/download/second/results{}.zip".format(num), "wb") as fout:
		for chunk in r2.iter_content(chunk_size=1024 * 1024 * 1024): # 15 GB
			if chunk:
				fout.write(chunk)
...
searches = ["species%20delimitation"]
